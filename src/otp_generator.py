...
class OTPGenerator:
    ...
    def generate_otp(self, secret: str, account_name: str = "Unknown") -> Dict[str, any]:
        """
        ワンタイムパスワードを生成
    ...
    def generate_multiple_otps(self, accounts: List[Dict[str, any]]) -> List[Dict[str, any]]:
        """
        複数のアカウントのOTPを生成
            ...
            try:
                if 'secret' in account:
                    otp_info = self.generate_otp(
                        account['secret'], 
                        account.get('account_name', 'Unknown')
                    )
    ...
    def start_realtime_display(self, accounts: List[Dict[str, any]], update_interval: int = 1):
        """
        リアルタイムでOTPを表示
    ...
    def _realtime_update_loop(self, accounts: List[Dict[str, any]], update_interval: int):
        """
        リアルタイム更新ループ
