...
class TestOneTimePasswordApp:
    ...
    def app(self):
        """テスト用アプリケーションインスタンス"""
        with patch('src.main.SecurityManager'), \
    ...
    def test_show_otp_success(self, app):
        ...
        mock_otps = [
            {'otp': '123456', 'account_name': 'test1@example.com', 'remaining_seconds': 15},
    ...
    def test_update_account_success(self, app):
        ...
        assert result is True
        app.security_manager.update_account.assert_called_once_with(account_id, **updates)
...
class TestMainFunction:
    ...
    def test_main_update_command(self):
        ...
        with patch('sys.argv', ['main.py', 'update', 'account-id', '--name', 'new-name']):
            with patch('src.main.OneTimePasswordApp') as mock_app_class:
                mock_app = Mock()
                mock_app_class.return_value = mock_app
                ...
                mock_app_class.assert_called_once()
                mock_app.update_account.assert_called_once_with('account-id', account_name='new-name')
