        ...
        with patch.object(docker_manager, '_validate_qr_url', return_value=True):
            with patch.object(docker_manager, 'ensure_image_available', return_value=True):
                with patch.object(docker_manager, 'run_container', return_value=(True, "otpauth://totp/test@example.com?algorithm=SHA1&digits=6&issuer=TestService&period=30&secret=JBSWY3DPEHPK3PXP")):
                    result = docker_manager.process_qr_url("otpauth-migration://offline?data=test")
...
class TestAccountFlowIntegration:
    ...
    def app(self, temp_data_dir):
        """テスト用アプリケーションインスタンス"""
        data_file = os.path.join(temp_data_dir, "test_accounts.json")
        with patch('src.main.SecurityManager') as mock_sm_class, \
    ...
    def test_concurrent_operations_integration(self, app):
        ...
        for i, account_id in enumerate(account_ids):
            success = app.security_manager.update_account(
                account_id, 
                account_name=f"updated_user{i}@example.com"
            )
            assert success is True
